...
class F10IniSelectionAdapter {
    ...
    Capture() {
        ...
        try {
            ...
            renderStart := startText + 0
            renderEnd := endText + 0
            if renderEnd <= renderStart
                return AQResult.Invalid("Selection range must be non-empty")
            return AQResult.Ok("Selection captured", Map(
                "render_session_id", sessionId,
...
class F10SourceMapContract {
    __New(data) {
        ...
        this.RenderSessionId := data["render_session_id"]
        this.SourceDocumentId := data["source_document_id"]
        this.SourcePath := data["source_path"]
    ...
    Resolve(renderStart, renderEnd) {
        ...
        ranges := this._SortedRanges()
        cursor := renderStart
        sourceRanges := []
        for item in ranges {
            rStart := item["render_start"]
            rEnd := item["render_end"]
            if rEnd <= cursor || rStart >= renderEnd
                continue
            overlapStart := Max(cursor, rStart)
            overlapEnd := Min(renderEnd, rEnd)
            ...
            renderLength := rEnd - rStart
            sourceLength := item["source_end"] - item["source_start"]
            ...
            offsetStart := overlapStart - rStart
            offsetEnd := overlapEnd - rStart
            sourceRanges.Push(Map(
                "start", item["source_start"] + offsetStart,
                "end", item["source_start"] + offsetEnd
            ))
            cursor := overlapEnd
            if cursor >= renderEnd
                break
        ...
        merged := this._MergeAdjacent(sourceRanges)
        return AQResult.Ok("Rendered selection resolved", Map("source_ranges", merged))
    }
...
class F10IniMapProvider {
    ...
    Load() {
        ...
        try {
            versionText := Trim(IniRead(this.Path, "map", "contract_version", ""))
            sessionId := Trim(IniRead(this.Path, "map", "render_session_id", ""))
            documentId := Trim(IniRead(this.Path, "map", "source_document_id", ""))
            sourcePath := IniRead(this.Path, "map", "source_path", "")
            sourceHash := Trim(IniRead(this.Path, "map", "source_hash", ""))
            countText := Trim(IniRead(this.Path, "map", "range_count", ""))
            if versionText != "1"
            ...
            ranges := []
            Loop countText + 0 {
                section := "range." A_Index
                rStart := this._UInt(section, "render_start")
                rEnd := this._UInt(section, "render_end")
                generated := IniRead(this.Path, section, "generated", "0") = "1"
                ...
                if generated {
                    ...
                    sStart := this._UInt(section, "source_start")
                    sEnd := this._UInt(section, "source_end")
            ...
            contract := F10SourceMapContract(Map(
                "contract_version", 1,
                "render_session_id", sessionId,
...
class F10FileSourceStore {
    Snapshot(path) {
        ...
        try {
            text := FileRead(path)
            return AQResult.Ok("Source snapshot read", Map(
                "text", text,
                "hash", F10Fingerprint.Hash(text),
                "length", StrLen(text)
            ))
        } catch as readError {
    ...
    DeleteRanges(path, expectedHash, ranges) {
        ...
        backupPath := path ".ahquiver.bak"
        tempPath := path ".ahquiver.tmp." A_TickCount
        try {
        ...
        return AQResult.Ok("Source ranges deleted", Map(
            "backup_path", backupPath,
            "old_hash", expectedHash,
            "new_hash", F10Fingerprint.Hash(text),
            "deleted_ranges", sorted.Length
        ))
    }
...
class F10EditService {
    ...
    Preview() {
        ...
        return AQResult.Ok("Source edit preview ready", Map(
            "preview_token", token,
            "render_session_id", contract.RenderSessionId,
            "source_document_id", contract.SourceDocumentId,
            "source_path", contract.SourcePath,
            "source_hash", contract.SourceHash,
            "render_start", selection.Data["render_start"],
            "render_end", selection.Data["render_end"],
            "source_ranges", resolved.Data["source_ranges"]
        ))
    }
    ...
    Delete(confirmText := "", previewToken := "") {
        ...
        return this.SourceStore.DeleteRanges(
            preview.Data["source_path"],
            preview.Data["source_hash"],
            preview.Data["source_ranges"]
        )
    }
...
class F10SourceMapEditModule {
    ...
    __New(selectionAdapter := unset, mapProvider := unset, sourceStore := unset) {
        this.InjectedSelection := IsSet(selectionAdapter) ? selectionAdapter : ""
        this.InjectedMapProvider := IsSet(mapProvider) ? mapProvider : ""
        this.InjectedSourceStore := IsSet(sourceStore) ? sourceStore : ""
        this.Service := ""
    ...
    Init(app) {
        ...
        store := IsObject(this.InjectedSourceStore) ? this.InjectedSourceStore : F10FileSourceStore()
        requireConfirmation := app.Config.GetBool("F10", "require_confirmation", true)
        ...
        status := (selection is F10UnsupportedSelectionAdapter) ? "unsupported" : "degraded"
        detail := status = "unsupported" ? "no cooperating selection adapter configured" : "cooperating files configured; validity checked on each action"
    ...
    ActionPreview(params) {
        ...
        if result.IsOk()
            ...
            this.App.Capabilities.Set("terminal.source_map_edit", "unsupported", result.Message)
        else
            this.App.Capabilities.Set("terminal.source_map_edit", "degraded", result.Message)
        return result
    }
    ActionDelete(params) {
        confirmText := params.Has("confirm") ? params["confirm"] : ""
        token := params.Has("preview_token") ? params["preview_token"] : ""
